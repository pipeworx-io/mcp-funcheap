...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Funcheap');
}
...
async function getEvent(args: Record<string, unknown>): Promise<unknown> {
  ...
  if (typeof args.id === 'number' || (typeof args.id === 'string' && /^\d+$/.test(args.id))) {
    try {
      ...
      if (/\b404\b/.test(msg)) {
        throw new Error(`Funcheap: no event with post id ${args.id}. Call search_events to find valid ids.`);
      }
      throw err;
    }
  }
  if (typeof args.slug === 'string' && args.slug.trim()) {
    ...
    if (!arr.length) throw new Error(`No Funcheap event found for slug "${args.slug}". Call search_events to find valid slugs.`);
